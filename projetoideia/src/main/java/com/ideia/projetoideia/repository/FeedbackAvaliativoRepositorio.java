...
public interface FeedbackAvaliativoRepositorio extends JpaRepository<FeedbackAvaliativo, Integer> {
	...
	@Query(value = "SELECT * from db_ideia.tb_feedback_avaliativo as feed where feed.lean_canvas_fk = ?1", nativeQuery = true)
	public List<FeedbackAvaliativo> findByLeanCanvas(Integer leanCanvas);
