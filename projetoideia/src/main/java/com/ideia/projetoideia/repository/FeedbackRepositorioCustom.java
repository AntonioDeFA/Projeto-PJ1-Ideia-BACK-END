...
public class FeedbackRepositorioCustom {
	...
	public List<FeedbackSugestaoDto> getByLeanCanvas(Integer id) {
		...
		StringBuffer jpql = new StringBuffer().append(
				"SELECT new com.ideia.projetoideia.model.dto.FeedbackSugestaoDto(feed.tipoFeedback, feed.sugestao, feed.dataCriacao, feed.id) FROM FeedbackAvaliativo AS feed WHERE feed.leanCanvas.id =:id");
